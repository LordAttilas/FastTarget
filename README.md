# FastTarget
Windower 4 addon for FFXI that allow easier target change.
...
Optional automatic best (nearest and in front) target search using the in-game sub-targeting system with F8 or custom key. Optional sub-target queue for automatic target switch when the current target die. Also track if a spell or job ability was used (using stnpc or st) to pull a different mob that then current one and set it automatically set as next queued target.
